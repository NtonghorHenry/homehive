...
export default function AddListing() {
    ...
    const { handleSubmit, handleChange, values, errors, touched } = useFormik({
        ...
        onSubmit: async (values,{resrtForm}) => {
            try {
                ...
                alert("Houses added succesfully")
                requestFormReset();
            }
            catch(error) {
        ...
        <main className="min-h-screen flex justify-center py-6 px-4">
            <Card sx={{ width: 380, height: 470 }}>
                <CardHeader
                    ...
                    subheader="Fill in the deatails below"
                />
                <CardContent>
                    <form onSubmit={handleSubmit} className="py-2 flex flex-col gap-3 px-2">
                        ...
                        <div>
                            <TextField
                                ...
                                placeholder="Deccribe the Property...."
                            />
                            {touched.description && errors.description ? <span className="text-xs text-red-500">{errors.description}</span> : null}
                        </div>
                        <button type="submit" className="w-full h-10 bg-blue-500 rounded-md shadow flex justify-center items-center text-white cursor-pointer font-semibold hover:opacity-50">Add Listing</button>
                    </form>
                </CardContent>
            </Card>
        </main>
